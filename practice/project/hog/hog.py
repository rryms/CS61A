...
def swine_align(player_score, opponent_score):
    ...
    Great_C_D=min(player_score,opponent_score)
    if player_score>0 and opponent_score>0:
        while(player_score%Great_C_D!=0 or opponent_score%Great_C_D!=0):
            Great_C_D-=1
    if (player_score>0 and opponent_score>0)and Great_C_D>=10:
        return True
    else:
        return False
    # END PROBLEM 4a
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score1<goal and score0<goal:
        if who==0:
            dice_sum=take_turn(strategy0(score0,score1),score1,dice)
            score0=score0+dice_sum
        elif who==1:
            dice_sum=take_turn(strategy1(score1,score0),score0,dice)
            score1=score1+dice_sum
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def announce(score0,score1):
        if who==0:
            minus=score0 - last_score
            runbest=running_high
            if minus>running_high:
                print(minus,"point(s)! The most yet for Player 0")
                runbest=minus
            return announce_highest(who,score0,runbest)
        if who==1:
            minus=score1 - last_score
            runbest=running_high
            if minus>running_high:
                print(minus,"point(s)! The most yet for Player 1")
                runbest=minus
            return announce_highest(who,score1,runbest)
    return announce
    # END PROBLEM 7
